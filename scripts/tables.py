...
for airline in database.cursor.execute(f"select * from Airline").fetchall():
...
for index, data in enumerate(file):
    ...
    print(airline_id)
    database.cursor.execute(f"insert into Airplane (type, airline) values ('{data[1]}', {airline_id})")
